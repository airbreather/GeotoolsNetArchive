...
namespace UrbansScience.Geographic.UnitTests.Geometries
{
	...
	/// </summary>
	public class MultiLineStringTest : TestCase
	{
		...
		/// <summary>
		/// Initializes a new instance of the MultiLineStringTest class.
		/// </summary>
		public MultiLineStringTest(String name) : base(name)
		{
		}
		...
		public void test_Constructor()
		{
			...
			AssertEquals("Constructor-1: ", false, multiLS.IsEmpty());
			AssertEquals("Constructor-2: ", false, multiLS2.IsEmpty());
		}
		...
		public void test_GeometryType()
		{
			...
			AssertEquals("GeometryType-1: ", "MultiLineString", multiLS.GetGeometryType());
		}
		...
		public void test_Coordinates()
		{
			...
			AssertEquals("Coordinates-1: ", 100, multiLS.GetCoordinates().Count);

			//now try it with a null geometry collection
			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
			multiLS = gf.CreateMultiLineString(null);

			AssertEquals("Coordinates-2: ", 0, multiLS.GetCoordinates().Count);

			//now try it with a different geometry collection
			multiLS = CreateMLS1();

			//1000 sets of coordinates
			AssertEquals("Cordinates-3: ", 1000, multiLS.GetCoordinates().Count);
		}
		...
		public void test_GetBoundaryDimension()
		{
			...
			AssertEquals("GetBoundaryDimension-1: ", 0, multiLS.GetBoundaryDimension());
			...
			//this returns a zero because it is not closed
			AssertEquals("GetBoundaryDimension-2: ", 0, multiLS.GetBoundaryDimension());

			//now try it with a closed multilinestring
			multiLS = closedMLS();

			//this returns a -1 because it is closed
			AssertEquals("GetBoundaryDimension-3: ", -1, multiLS.GetBoundaryDimension());
		}
		...
		public void test_EqualExact()
		{
			...
			AssertEquals("Equals-1: ", true , multiLS1.Equals(multiLS5));
			AssertEquals("Equals-2: ", false, multiLS1.Equals(multiLS2));
			AssertEquals("Equals-3: ", false, multiLS1.Equals(multiLS3));
			AssertEquals("Equals-4: ", false, multiLS1.Equals(multiLS4));
		}
...
//			AssertEquals("CompareToSameClass1: ", 0, multiLS1.CompareToSameClass(multiLS1));
//			AssertEquals("CompareToSameClass2: ", -1, multiLS1.CompareToSameClass(multiLS3));
//			AssertEquals("CompareToSameClass3: ", 1, multiLS3.CompareToSameClass(multiLS1));
		}
...
//			AssertEquals("ComputeEnvelopeInternal-1: ", 0.0, multiLS1.ComputeEnvelopeInternal().MinX);
//			AssertEquals("ComputeEnvelopeInternal-2: ", 5.0, multiLS1.ComputeEnvelopeInternal().MinY);
//			AssertEquals("ComputeEnvelopeInternal-3: ", 18.0, multiLS1.ComputeEnvelopeInternal().MaxX);
//			AssertEquals("ComputeEnvelopeInternal-4: ", 23.0, multiLS1.ComputeEnvelopeInternal().MaxY);
		}
		...
		public void test_Clone()
		{
			...
			AssertEquals("Clone: ", true, multiLS1.Equals(multiLS2));
		}
		...
		public void test_IsEmpty()
		{
			...
			AssertEquals("IsEmpty-1: ", false, multiLS.IsEmpty());

			//now try it with a null geometry collection
			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
			multiLS = gf.CreateMultiLineString(null);

			AssertEquals("IsEmpty-2: ", true, multiLS.IsEmpty());

			//now try it with a different geometry collection
			multiLS = CreateMLS1();

			AssertEquals("IsEmpty-3: ", false, multiLS.IsEmpty());

			//now try it again
			multiLS = closedMLS();

			AssertEquals("IsEmpty-4: ", false, multiLS.IsEmpty());
		}
		...
		public void test_Dimension()
		{
			...
			AssertEquals("Dimension: ", 1, multiLS.GetDimension());
		}
		...
		public void test_IsSimple()
		{
			...
			AssertEquals("IsSimple-1: ", false, multiLS.IsSimple());

			//now try it with a null geometry collection
			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
			multiLS = gf.CreateMultiLineString(null);

			AssertEquals("IsSimple-2: ", true, multiLS.IsSimple());

			//now try it with a different geometry collection
			multiLS = CreateMLS1();

			//TODO: This is really slow!!!!!!!!!  Why?
			//AssertEquals("IsSimple-3: ", false, multiLS.IsSimple());

			//now try it with a mixed geometry collection
			multiLS = nonSimpleMLS();

			AssertEquals("IsSimple-4: ", false, multiLS.IsSimple());

			//now try it with a closed geometry collection
			multiLS = closedMLS();

			//TODO: Uncomment when IsSimple is working.
			AssertEquals("IsSimple-5: ", true, multiLS.IsSimple());
		}
		...
		public void test_NumPoints()
		{
			...
			AssertEquals("NumPoints-1: ", 100, multiLS.GetNumPoints());

			//now try it with a null geometry collection
			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
			multiLS = gf.CreateMultiLineString(null);

			AssertEquals("NumPoints-2: ", 0, multiLS.GetNumPoints());

			//now try it with a different geometry collection
			multiLS = closedMLS();

			AssertEquals("NumPoints-3: ", 30, multiLS.GetNumPoints());

			//now try it with a mixed geometry collection
			multiLS = nonSimpleMLS();

			AssertEquals("NumPoints-4: ", 18, multiLS.GetNumPoints());
		}
...
//			AssertEquals("Extent2D-2: ", 5.0, minY);
//			AssertEquals("Extent2D-3: ", 18.0, maxX);
//			AssertEquals("Extent2D-4: ", 23.0, maxY);
//
//			//now try it with a null geometry collection
//			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
//			multiLS = gf.CreateMultiLineString(null);
...
//			AssertEquals("Project-1: ", null, multiLS.Project(null));
		}
...
//			MultiLineString multiLS = CreateMLS();
//
//			AssertEquals("SetEmpty-1: ", false, multiLS.IsEmpty);
//			try
//			{
//				multiLS.SetEmpty();
...
//			}
		...
		public void test_Geometry()
		{
			...
			AssertEquals("Geometry-1: ", "LineString:(5, 3, NaN),(4, 5, NaN),(3, 4, NaN)", mls.GetGeometryN(0).ToString());
			AssertEquals("Geometry-2: ", "LineString:(2, 7, NaN),(9, 2, NaN),(7, 9, NaN)", mls.GetGeometryN(1).ToString());
		}
		...
		public void test_Envelope()
		{
			...
			AssertEquals("Envelope-3: ", 5.0, coords[0].Y);

			//check the second set of coordinates (maxX, minY)
			AssertEquals("Envelope-4: ", 18.0, coords[1].X);
			AssertEquals("Envelope-5: ", 5.0, coords[1].Y);

			//check the third set of coordinates (maxX, maxY)
			AssertEquals("Envelope-6: ", 18.0, coords[2].X);
			AssertEquals("Envelope-7: ", 23.0, coords[2].Y);

			//check the forth set of coordinates (minX, maxY)
			AssertEquals("Envelope-8: ", 0.0, coords[3].X);
			AssertEquals("Envelope-9: ", 23.0, coords[3].Y);

			//check the fifth set of coordinates (minX, minY)
			AssertEquals("Envelope-10: ", 0.0, coords[4].X);
			AssertEquals("Envelope-11: ", 5.0, coords[4].Y);
		...
		public void test_NumGeometries()
		{
			...
			AssertEquals("NumGeometries-1: ", 10, multiLS.GetNumGeometries());

			//now try it with a null geometry collection
			GeometryFactory gf = new GeometryFactory(_precMod, _sRID);
			multiLS = gf.CreateMultiLineString(null);

			AssertEquals("NumGeometries-2: ", 0, multiLS.GetNumGeometries());

			//now try it with a different geometry collection
			multiLS = closedMLS();

			AssertEquals("NumGeometries-3: ", 2, multiLS.GetNumGeometries());

			//now try it with a mixed geometry collection
			multiLS = nonSimpleMLS();

			AssertEquals("NumGeometries-4: ", 1, multiLS.GetNumGeometries());
		}
