...
namespace Geotools.IO
{
	...
	public class DbaseFieldDescriptor
	{
		...
		public static char GetDbaseType(Type type)
		{
			...
			throw new NotSupportedException(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0} does not have a corresponding dbase type.",type.Name));
		}
		...
		public DbaseFieldDescriptor(string name)
		{
			_name = name;
		}

		public DbaseFieldDescriptor(string name, char dbaseType)
			: this(name)
		{
			_type = dbaseType;
		}

		public DbaseFieldDescriptor(string name, char dbaseType, int length)
			: this(name, dbaseType)
		{
			_length = length;
		}

		public DbaseFieldDescriptor(string name, char dbaseType, int length, int decimalCount)
			: this(name, dbaseType, length)
		{
