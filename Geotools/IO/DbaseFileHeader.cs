...
namespace Geotools.IO
{
	...
	public class DbaseFileHeader
	{
		...
		public int RemoveColumn (string fieldName) 
		{
			...
			for (int i = 0, j = 0; i < _fieldDescriptions.Length; i++) 
			{
				if (fieldName.ToLower()!=(_fieldDescriptions[i].Name.Trim().ToLower())) 
				{
					...
					// named field
					if (i == j &&
						i == _fieldDescriptions.Length - 1) 
					{
						Trace.WriteLineIf(Shapefile.TraceSwitch.Enabled,"Could not find a field named '" + fieldName + "' for removal");
						return retCol;
					}
		...
		internal void SetNumRecords(int inNumRecords)
		{
