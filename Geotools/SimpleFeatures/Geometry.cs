...
{
	/// <summary>
	/// A set of points. The spatial relationship predicates (like disjoint) 
	...
	{	
		...
		{
			if ( !_precisionModel.Equals( geometry.PrecisionModel ) ) 
			{
		...
		public virtual Geometry Project(ICoordinateTransformation  coordinateTransform)
		{
			 ...
			 * could not make this abstract - otherwise go an error.
			   [nunit] Running Geotools.UnitTests.AllTests
				[nunit] ERROR: ProjectTest.Test1:\projects\ogis v. 1.0\4 construction\Geotools.unittests\geometres\projecttest.cs(56): Could not load type Geotools.Geometries.Gometry from assembly UrbanScience.Geographic, Version=0.0.0.0, Culture=neutral,PublicKeyToken=null.
		...
		public virtual void Extent2D(out double minX, out double minY, out double maxX, out double maxY)
		{
			...
			Coordinate externalMin = this._precisionModel.ToExternal(min);
			Coordinate externalMax = this._precisionModel.ToExternal(max);
			minX = externalMin.X;
			minY = externalMin.Y;
			maxX = externalMax.X;
			maxX = externalMax.Y;
