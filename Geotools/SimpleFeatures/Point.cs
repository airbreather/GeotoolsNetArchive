...
namespace Geotools.SimpleFeatures
{
	...
	public class Point : Geometry, IPoint
	{
		...
		public override Geometry Project(ICoordinateTransformation  coordinateTransform)
		{
			...
			Coordinate projectedCoordinate = this._precisionModel.ToExternal(this._coordinate);

			double x=0.0;
			double y=0.0;
			if (this.GetSRID()==sourceSRID)
			{
				projection.MetersToDegrees(projectedCoordinate.X,projectedCoordinate.Y, out x, out y);
			}
			else if (this.GetSRID()==targetSRID)
			{
				projection.DegreesToMeters(projectedCoordinate.X,projectedCoordinate.Y, out x, out y);
			}
			
			Coordinate internalCoordinate = new Coordinate(x,y);
			projectedCoordinate = this._precisionModel.ToInternal(internalCoordinate);
			return new Point( projectedCoordinate, this.PrecisionModel, newSRID);
		} // public override IGeometry Project(OGC.CoordinateTransformations.CT_CoordinateTransformation  coordinateTransform)	
